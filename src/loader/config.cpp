...
namespace
{
    ...
    ConfigData BuildDefaultConfig(const StartupOptions& options, const IApplication& application)
    {
        ConfigData config;
        config.config_path = ResolveConfigPath(options, application);
        config.pid_file = ResolvePidFilePath(options, application);
        config.log_file = ResolveLogFilePath(options, application);
        config.error_log_file = ResolveErrorLogFilePath(options, application);
        return config;
    }

    bool LoadYamlConfig(ConfigData& config, bool config_path_explicit, bool verbose)
    {
        ...
        if (!ApplyString(root["log"]["level"], "log.level", config.log_level) ||
            !ApplyString(root["log"]["file"], "log.file", config.log_file) ||
            !ApplyString(root["log"]["error_file"], "log.error_file", config.error_log_file) ||
            !ApplyBool(root["log"]["console"], "log.console", config.log_to_console) ||
            !ApplyBool(root["log"]["syslog"], "log.syslog", config.log_to_syslog) ||
            !ApplyString(root["log"]["rotate"]["mode"], "log.rotate.mode", config.log_rotation_mode) ||
            !ApplySizeValue(root["log"]["rotate"]["max_size"], "log.rotate.max_size", config.log_max_size) ||
            !ApplyUnsigned(root["log"]["rotate"]["max_files"], "log.rotate.max_files", config.log_max_files) ||
            !ApplyUnsigned(root["log"]["rotate"]["daily_hour"], "log.rotate.daily_hour", config.log_rotate_hour) ||
            !ApplyUnsigned(root["log"]["rotate"]["daily_minute"], "log.rotate.daily_minute", config.log_rotate_minute) ||
            !ApplyBool(root["runtime"]["daemon"], "runtime.daemon", config.daemon) ||
            !ApplyString(root["runtime"]["pid_file"], "runtime.pid_file", config.pid_file))
        {
            return false;
        }
    ...
    void ApplyCliOverrides(ConfigData& config, const StartupOptions& options)
    {
        ...
        {
            config.pid_file = options.pid_file;
        ...
        {
            config.error_log_file = options.error_log_file;
        ...
        {
            config.log_rotate_minute = options.log_rotate_minute;
            SetSetting(config.settings, "log.rotate.daily_minute", std::to_string(options.log_rotate_minute));
        }
    }

    bool ValidateConfig(const ConfigData& config)
    {
        const std::string mode = ToLower(config.log_rotation_mode);
    ...
    void CopyToContext(const ConfigData& config, ApplicationContext& context)
    {
        context.config_path = config.config_path;
        context.pid_file = config.pid_file;
        context.log_file = config.log_file;
        context.error_log_file = config.error_log_file;
        context.log_level = config.log_level;
        context.log_rotation_mode = ToLower(config.log_rotation_mode);
        context.log_max_size = config.log_max_size;
        context.log_max_files = config.log_max_files;
        context.log_rotate_hour = config.log_rotate_hour;
        context.log_rotate_minute = config.log_rotate_minute;
        context.daemon = config.daemon;
        context.log_to_console = config.log_to_console;
        context.log_to_syslog = config.log_to_syslog;
...
{
    ConfigData config = BuildDefaultConfig(options, application);
